    ...
    real(mcp) Params(:)
    class(TTheoryParams), target :: CMB
    real(mcp) omegam, h2, value
    integer ::i 

    real (dl), dimension(2):: temp
!print*, 'here'
    select type (CMB)
    class is (CMBParams)
            CMB%omb = Params(1)
...
!            if ( CosmoSettings%use_flatness==0 ) then
                CMB%omv = Params(3)
!            else if ( CosmoSettings%use_flatness==1 ) then
!                CMB%omv = 1._mcp -CMB%omc -CMB%omb -CMB%omnu
!            else if ( CosmoSettings%use_flatness==2 ) then
!                CMB%omv = 1._mcp -CMB%omc -CMB%omb -CMB%omnu
!
!            end if
	...
	!SP: modification for f(R) log sampling
temp(1)	= 10.**Params(10)
temp(2)	= Params(11)
!temp(3)	= Params(12)
                call CMB%EFTCAMB_parameters%model%init_model_parameters( temp )
!i=1
!call CMB%EFTCAMB_parameters%model%parameter_values(i, value)
!write(*,*) value
            end if
#endif
            ! EFTCosmoMC MOD END.
    ...
    class(BackgroundParameterization) :: this
    real(mcp), allocatable :: derived(:), derived_sigma(:,:), derived_mu(:,:),derived_sigma_exact(:,:), derived_mu_exact(:,:), derived_Cs_square(:)
 
    class(TTheoryPredictions), allocatable :: Theory
    real(mcp) :: P(:)
    real(mcp) :: a, k, value
    real (mcp) :: scale_factor_initial, scale_factor_final, k_initial, k_final
    integer :: i,j, iter, tot, l, num_w, num_k
